...
from .forms import ProficiencyForm
...
def proficiency_calculator(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Calculate score based on selected levels
            for field, selected_level in form.cleaned_data.items():
                if selected_level:
                    skill_id = field.split('_')[1]
                    skill = ProficiencySkill.objects.get(id=skill_id, level=selected_level)
                    total_score += skill.weight
                    selected_levels.append((skill.category, skill.level, skill.weight))
